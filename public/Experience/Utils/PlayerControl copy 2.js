...
export default class PlayerControl extends EventEmitter
{
    ...
    update(delta) {
      ...
      if (this.model.animation.actions.current == this.model.animation.actions.run_forward || this.model.animation.actions.current == this.model.animation.actions.walk_forward) {
        ...
        let diffX = box.min.x - copyBox.min.x
        let diffZ = box.min.z - copyBox.min.z
    ...
    checkMovement(bb) {
        // Calculate the minimum and maximum indices in the x and y directions
        const minX = Math.floor((bb.min.x + this.experience.world.gridSize.x/2) / this.experience.world.gridSize.x);
        const maxX = Math.floor((bb.max.x + this.experience.world.gridSize.x/2) / this.experience.world.gridSize.x);
        const minY = Math.floor((bb.min.z + this.experience.world.gridSize.z/2) / this.experience.world.gridSize.z);
        const maxY = Math.floor((bb.max.z + this.experience.world.gridSize.z/2) / this.experience.world.gridSize.z);
    
        // Nested loop to iterate over the cells in the bounding box
        for (let x = minX*2; x <= maxX*2; x++) {
            for (let y = minY*2; y <= maxY*2; y++) {
                // Check if the cell value is 0
                if (this.experience.world.matrix[x][y] === 0) {
